<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_do_donation d WHERE p.isdonner='Donner'");
            try
            {
                $isCasepaper = filter_input(INPUT_GET, 'isCasepaper');
                if($isCasepaper==='1')
                    $query = $query . " and p.person_id  = d.person_id";
            }
            catch(Exception $e){}
        ...
        if($process === "getSelected_donation_data")
		{
			$do_donation_id=filter_input(INPUT_GET, 'do_donation_id');
            
			//echo "6";
            
			$query = ("SELECT do_payment_date, donation_paid, donation_towards, payment_mode, payment_details, pan_number, pan_name FROM ak_do_donation WHERE do_donation_id= '" . $do_donation_id . "'");
