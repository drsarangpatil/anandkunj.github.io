<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_case_sheet c, ak_rt_room_allocation r WHERE c.ad_status='Admitted' and p.isparticipant='Participant'");
            try
            {
        ...
        if($process === "getRt_room_allocation_record")
		{
			$rt_case_sheet_id=filter_input(INPUT_GET, 'rt_case_sheet_id');
             
            //echo $rt_case_sheet_id;
             
            $query = ("SELECT Distinct p.full_name, r.rt_room_allocation_id, r.building_name, r.room_number, r.room_status, r.bed_number, r.doa, r.doc, r.retreat_duration, r.attendant_name1, r.bed_number1, r.attendant_name2, r.bed_number2, p.person_id FROM ak_p_registration p, ak_rt_room_allocation r, ak_rt_case_sheet c WHERE c.ad_status='Admitted'");
             
            $query = $query . " and p.person_id  = c.person_id";
            $query = $query . " and c.rt_case_sheet_id  = r.rt_case_sheet_id";
            $query = $query . " and r.rt_case_sheet_id  = '" . $rt_case_sheet_id ."'";
        ...
        if($process === "getSelected_Rt_room_allocation_record")
		{
           ...
           //$rt_case_sheet_id=filter_input(INPUT_GET, 'rt_case_sheet_id');
			...
			$query = ("SELECT  rt_room_allocation_id, p_type, building_name, room_number, bed_number, room_status, no_attendant, attendant_name1, bed_number1, attendant_name2, bed_number2 FROM ak_rt_room_allocation WHERE rt_room_allocation_id = '" . $rt_room_allocation_id ."'");
