<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_case_sheet c, ak_rt_valuables_deposit_received b WHERE b.v_d_status='Received' and p.isparticipant='Participant'");
            try
            {
       ...
       if($process === "getAll_deposit_records")
		{
			$rt_case_sheet_id=filter_input(INPUT_GET, 'rt_case_sheet_id');
            
			//echo "6";
            
            $query = ("SELECT rt_v_d_received_id, deposit_amount, v_items, v_d_status, rt_v_d_received_date, schedule,  user_id  FROM ak_rt_valuables_deposit_received WHERE v_d_status='Received' and rt_case_sheet_id = '" . $rt_case_sheet_id ."'");
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        $row= $row .'<td name=rt_v_d_received_id align="center">' . $record[0] . '</td>';
                        $row= $row .'<td name=deposit_amount align="center">' . $record[1] . '</td>';
                        $row= $row .'<td name=v_items align="left">' . $record[2] . '</td>';
                        $row= $row .'<td name=v_d_status align="center">' . $record[3] . '</td>';
                        $row= $row .'<td name=rt_v_d_received_date align="left">' . $record[4] . '</td>';
                        $row= $row .'<td name=schedule align="center">' . $record[5] . '</td>';
        ...
        if($process === "getSelected_deposit_data")
		{
			$rt_v_d_received_id=filter_input(INPUT_GET, 'rt_v_d_received_id');
			//echo "6";
            
			$query = ("SELECT deposit_amount, v_items, rt_v_d_received_date, schedule, v_d_status  FROM ak_rt_valuables_deposit_received WHERE rt_v_d_received_id = '" . $rt_v_d_received_id. "'");
