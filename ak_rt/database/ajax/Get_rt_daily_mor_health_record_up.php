<?php
	...
	if ($con)
	{
        ...
        if($process === "getAll_rt_treatment_records")
		{
            ...
            $query = ("SELECT daily_date, treatment_name, therapy_dept, treatment_time, therapist_name, treat_instructions, schedule, rt_treatment_prescription_id FROM ak_rt_daily_treatment_prescription WHERE rt_case_sheet_id = '" . $rt_case_sheet_id ."' and schedule='Morning'");
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        ...
                        $row= $row .'<td name=rt_treatment_prescription_id align="left">' . $record[7] . '</td>';
                    $row= $row .'</tr>';
        ...
        if($process === "getSelected_rt_treatment_records")
		{
			...
			$query = "SELECT treatment_name, therapy_dept, treatment_time, therapist_name, treat_instructions FROM ak_rt_daily_treatment_prescription WHERE schedule= 'Morning' and daily_date = '" . $daily_date ."'";
            $query = $query . " and rt_case_sheet_id = '" . $rt_case_sheet_id ."'";
        ...
        if($process === "getAll_rt_diet_records")
		{
            ...
            $query = "SELECT daily_date, morning_diet, noon_diet, afternoon_diet, evening_diet, instruc, schedule, rt_diet_prescription_id FROM ak_rt_daily_diet_prescription WHERE rt_case_sheet_id = '" . $rt_case_sheet_id ."'";
            $query = $query . " and schedule ='Morning'";
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        ...
                        $row= $row .'<td name=rt_diet_prescription_id align="center">' . $record[7] . '</td>';
                    $row= $row .'</tr>';
         ...
         if($process === "getSelected_rt_diet_records")
		{
			...
			$query = "SELECT morning_diet, noon_diet, afternoon_diet, evening_diet, instruc FROM ak_rt_daily_diet_prescription WHERE schedule= 'Morning' and daily_date = '" . $daily_date ."'";
            $query = $query . " and rt_case_sheet_id = '" . $rt_case_sheet_id ."'";
