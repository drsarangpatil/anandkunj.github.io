<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_case_sheet c, ak_rt_billing b WHERE b.payment_status='Due' and p.isparticipant='Participant'");
            try
            {
                $isCasepaper = filter_input(INPUT_GET, 'isCasepaper');
                if($isCasepaper==='1')
                    $query = $query . " and p.person_id  = c.person_id";
                    $query = $query . " and c.person_id  = b.person_id";
            }
            catch(Exception $e){}
        ...
        if($process === "getAll_billing_records")
		{
            ...
            $query = "SELECT rt_estimation_id, payable_amount, rt_case_sheet_id, rt_estimate_date, schedule,  user_id  FROM ak_rt_billing";
            $query =  $query . " WHERE rt_estimation_id  = ";
            $query =  $query . " (SELECT max(rt_estimation_id) FROM ak_rt_billing WHERE";
            $query =  $query . " rt_case_sheet_id = " .$rt_case_sheet_id ." and payment_status='Due')";
            
            
            /*$query = ("SELECT rt_estimation_id, payable_amount, rt_case_sheet_id, rt_estimate_date, schedule,  user_id  FROM ak_rt_billing WHERE payment_status='Due' and rt_case_sheet_id = '" . $rt_case_sheet_id ."'");*/
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        $row= $row .'<td name=rt_estimation_id align="center">' . $record[0] . '</td>';
                        $row= $row .'<td name=payable_amount align="center">' . $record[1] . '</td>';
                        $row= $row .'<td name=rt_case_sheet_id align="center">' . $record[2] . '</td>';
                        $row= $row .'<td name=rt_estimate_date align="center">' . $record[3] . '</td>';
                        $row= $row .'<td name=schedule align="center">' . $record[4] . '</td>';
		...
		{
			$rt_estimation_id=filter_input(INPUT_GET, 'rt_estimation_id');
			//echo "6";
            
			$query = ("SELECT rt_estimate_date, schedule, retreat_donation, attendant_donation, accommodation_donation, food_donation, other_donation, discount_amount, payable_amount, payment_status, pan_name, pan_number FROM ak_rt_billing WHERE rt_estimation_id = '" . $rt_estimation_id. "'");
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
					$option  = $record[0] . '~~' . $record[1] . '~~' . $record[2] . '~~' . $record[3] . '~~' . $record[4] . '~~' . $record[5] . '~~' . $record[6] . '~~' . $record[7] . '~~' . $record[8] . '~~' . $record[9] . '~~' . $record[10] . '~~' . $record[11];
				}
				echo $option;			// send data back to JS file
