<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_case_sheet c, ak_rt_daily_health_record d, ak_rt_daily_treatment_prescription t WHERE c.ad_status='Admitted' and p.isparticipant='Participant'");
            $query = $query . " and t.schedule= 'Afternoon'";
            try
            {
                ...
                if($isPrescription==='1')
                    ...
                    $query = $query . " and d.rt_case_sheet_id  = t.rt_case_sheet_id";
            }
            catch(Exception $e){}
		...
		{
			$daily_date=filter_input(INPUT_GET, 'daily_date');
            $rt_case_sheet_id=filter_input(INPUT_GET, 'rt_case_sheet_id');
            
			//echo "6";
            
			$query = "SELECT retreat_day, daily_date, schedule, weight, bp, amroli, water, detox, motions, complaints, oe, diet_status, daily_medicine FROM ak_rt_daily_assessment_record WHERE schedule= 'Afternoon' and daily_date = '" . $daily_date ."'";
        ...
        if($process === "getSelected_rt_treatment_records")
		{
			...
			$query = "SELECT treatment_name, therapy_dept, treatment_time, therapist_name, treat_instructions FROM ak_rt_daily_treatment_prescription WHERE schedule= 'Afternoon' and daily_date = '" . $daily_date ."'";
            $query = $query . " and rt_case_sheet_id = '" . $rt_case_sheet_id ."'";
