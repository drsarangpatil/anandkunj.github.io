<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_case_sheet c, ak_rt_payment r WHERE r.payment_status='Partial' and p.isparticipant='Participant'");
            try
            {
        ...
        if($process === "getAll_payment_records")
		{
            ...
            $query = "SELECT rt_payment_id, payable_amount, rt_payment_date, amount_paid, payment_mode, balance_amount, user_id FROM ak_rt_payment";
            $query =  $query . " WHERE rt_payment_id  = ";
            $query =  $query . " (SELECT max(rt_payment_id) FROM ak_rt_payment WHERE";
            $query =  $query . " rt_case_sheet_id = " .$rt_case_sheet_id ." and payment_status='Partial')";
        ...
        if($process === "getSelected_payment_data")
		{
			...
			$query = ("SELECT rt_estimation_id, balance_amount, pan_name, pan_number FROM ak_rt_payment WHERE rt_payment_id = '" . $rt_payment_id. "'");
