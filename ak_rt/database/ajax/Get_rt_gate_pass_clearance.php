<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_discharge_sheet c WHERE c.ad_status !='Admitted' and p.isparticipant='Participant'");
            try
            {
        ...
        if($process === "getGatepass_record")
		{
			...
			$query = ("SELECT rt_gatepass_id, gatepass_date, schedule, acc_d_clear, hk_d_clear, shop_clear, sub_clear, mem_clear, lib_clear, v_t_clear, off_clear FROM ak_rt_gatepass WHERE rt_case_sheet_id = '" . $rt_case_sheet_id ."'");
