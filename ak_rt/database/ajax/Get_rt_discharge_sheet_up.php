<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_case_sheet c, ak_rt_discharge_sheet d WHERE c.ad_status !='Admitted' and p.isparticipant='Participant'");
            try
            {
        ...
        if($process === "getAll_discharge_sheet_records")
		{
			$person_id=filter_input(INPUT_GET, 'person_id');
            
			//echo "6";
            
            $query = ("SELECT ak_rt_discharge_sheet_id, rt_case_sheet_id, discharge_date, ad_status, user_id FROM ak_rt_discharge_sheet WHERE person_id  = '" . $person_id ."'");
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        $row= $row .'<td name=ak_rt_discharge_sheet_id align="center">' . $record[0] . '</td>';
                        $row= $row .'<td name=rt_case_sheet_id align="center">' . $record[1] . '</td>';
                        $row= $row .'<td name=discharge_date align="center">' . $record[2] . '</td>';
                        $row= $row .'<td name=ad_status align="center">' . $record[3] . '</td>';
        ...
        if($process === "getRetreat_details")
		{
            ...
            $query = ("SELECT doa, retreat_name, retreat_duration, doc, ad_status, rt_case_sheet_id FROM ak_rt_case_sheet WHERE rt_case_sheet_id = '" . $rt_case_sheet_id ."'");
        ...
        if($process === "getRoom_details")
		{
            ...
            $query = ("SELECT room_number, room_status FROM ak_rt_room_allocation WHERE rt_case_sheet_id = '" . $rt_case_sheet_id ."'");
        ...
        if($process === "getSelected_Rt_discharge_record")
		{
			$ak_rt_discharge_sheet_id=filter_input(INPUT_GET, 'ak_rt_discharge_sheet_id'); 
            //$rt_case_sheet_id=filter_input(INPUT_GET, 'rt_case_sheet_id');
            
            
			//echo "6";
            
			$query = ("SELECT  ak_rt_discharge_sheet_id, discharge_date, schedule FROM ak_rt_discharge_sheet WHERE ak_rt_discharge_sheet_id = '" . $ak_rt_discharge_sheet_id ."'");
