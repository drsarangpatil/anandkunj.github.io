<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_rt_case_sheet c, ak_rt_valuables_deposit_received b WHERE b.v_d_status='Partial' and p.isparticipant='Participant'");
            try
            {
       ...
       if($process === "getAll_deposit_returned_records")
		{
			$rt_case_sheet_id=filter_input(INPUT_GET, 'rt_case_sheet_id');
            
			//echo "6";
            
            $query = ("SELECT rt_v_d_returned_id, deposit_amount, returned_amount, balance_amount,  v_items, rt_v_d_returned_date, user_id FROM ak_rt_valuables_deposit_returned WHERE v_d_status='Partial' and rt_case_sheet_id = '" . $rt_case_sheet_id ."'");
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        $row= $row .'<td name=rt_v_d_returned_id align="center">' . $record[0] . '</td>';
                        $row= $row .'<td name=deposit_amount align="center">' . $record[1] . '</td>';
                        $row= $row .'<td name=returned_amount align="left">' . $record[2] . '</td>';
                        $row= $row .'<td name=balance_amount align="center">' . $record[3] . '</td>';
                        $row= $row .'<td name=v_items align="left">' . $record[4] . '</td>';
                        $row= $row .'<td name=rt_v_d_returned_date align="center">' . $record[5] . '</td>';
        ...
        if($process === "getSelected_deposit_returned_data")
		{
			$rt_v_d_returned_id=filter_input(INPUT_GET, 'rt_v_d_returned_id');
			//echo "6";
            
			$query = ("SELECT rt_v_d_received_id, balance_amount, v_items FROM ak_rt_valuables_deposit_returned WHERE rt_v_d_returned_id = '" . $rt_v_d_returned_id. "'");
