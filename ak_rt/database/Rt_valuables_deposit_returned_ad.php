<?php
	...
	if ($con)
	{
		...
		
		{
            ...
            $query1= "UPDATE ak_rt_valuables_deposit_returned SET v_d_status='". $_POST["v_d_status"]."' WHERE rt_v_d_received_id='" . $_POST["rt_v_d_received_id"] ."'";
			...
			if (mysqli_affected_rows($con))
			{		
				echo "<script> alert('Deposit Returned Status Updated')</script>";
			}
			else	
				echo "<script> alert('Deposit Returned Status  Not Updated')</script>";
			...
			if (mysqli_affected_rows($con))
			{		
				echo "<script> alert('Deposit Received Status Updated')</script>";
			}
			else	
				echo "<script> alert('Deposit Received Status  Not Updated')</script>";
	...
	echo "<script> location.replace('../rt_valuables_deposit_returned_ad.php'); </script>";
