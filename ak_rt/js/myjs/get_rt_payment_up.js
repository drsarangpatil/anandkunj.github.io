...
{	
    ...
    (
        {
            url: 'database/ajax/Get_rt_payment_up.php',	// call page for data to be retrived
...
( 
    function() 
    {
        ...
        (
            {
                url: 'database/ajax/Get_rt_payment_up.php',
                type: 'GET',							
                                                ...
                                                (
                                                    {
                                                        url: 'database/ajax/Get_rt_payment_up.php',// call page for data to be retrived
                                                        type: 'GET',				// to get data in backgrouond
                                                        data: { process: "getAll_payment_records", rt_case_sheet_id:response[0]},	
    ...
    function myshow()
        {
            var rt_payment_id = $('#rt_payment_id').val();
            ...
            (
                {
                    url: 'database/ajax/Get_rt_payment_up.php',
                    type: 'GET',							
                    data: { process: "getSelected_payment_data", rt_payment_id:rt_payment_id },

                    success: function (data) 
                    {
                        var res = data.split("~~"); // to split the fetched data
                        $("#rt_payment_date").val(res[0]);
                        $("#schedule").val(res[1]);
                        $("#rt_estimation_id").val(res[2]);
                        $("#payable_amount").val(res[3]);
                        $("#amount_paid").val(res[4]);
                        $("#balance_amount").val(res[5]);
                        $("#payment_mode").val(res[6]);
                        $("#payment_details").val(res[7]);
                        $("#payment_status").val(res[8]); 
                        $("#pan_name").val(res[9]); 
                        $("#pan_number").val(res[10]);
