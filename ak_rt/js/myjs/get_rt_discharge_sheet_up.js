...
{	
    ...
    (
        {
            url: 'database/ajax/Get_rt_discharge_sheet_up.php',	// call page for data to be retrived
...
( 
    function() 
    {
        ...
        (
            {
                url: 'database/ajax/Get_rt_discharge_sheet_up.php',
                type: 'GET',							
                                    ...
                                    (
                                        {
                                            url: 'database/ajax/Get_rt_discharge_sheet_up.php',// call page for data to be retrived
                                            type: 'GET',				// to get data in backgrouond
                                            data: { process: "getAll_discharge_sheet_records", person_id:res[0]},

                                            success: function (data) 
                                            {
                                               //alert("Success : " + data);

                                               $("#old_discharge_records_list").html(data);
...
function myshow()
    {
        ...
        var ak_rt_discharge_sheet_id = $('#ak_rt_discharge_sheet_id').val();
        //alert(ak_rt_discharge_sheet_id);
        $.ajax
        (
            {
                url: 'database/ajax/Get_rt_discharge_sheet_up.php',// call page for data to be retrived
                type: 'GET',							     // to get data in backgrouond
                data: { process: "getSelected_Rt_discharge_record", ak_rt_discharge_sheet_id:ak_rt_discharge_sheet_id},	
                ...
                success: function (data) 
                {
                    ...
                    $("#ak_rt_discharge_sheet_id").val(res[0]);
                    $("#discharge_date").val(res[1]);
                    $("#schedule").val(res[2]);
