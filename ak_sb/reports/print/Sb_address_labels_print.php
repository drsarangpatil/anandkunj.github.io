<html lang="en">
    <head>
        <title> Print SB Address Labels</title>
        <meta charset="utf-8">
...
<!-- Php code for OP Case Sheet Table Starts =============================== -->
                            <?php 
                                ...
                                {
                                    {
                                        $query="SELECT s.sb_subscription_id, s.dos, s.doc, s.sub_status, p.photo, p.full_name, p.address, p.at_post, p.pin_code, p.state_name, p.mobile_no, s.magazine_name, s.subscription_type FROM ak_p_registration p, ak_sb_subscription_form s WHERE p.person_id= s.person_id ";
                                       ...
                                       /*$i =1;*/
                                        while($record = mysqli_fetch_array($response))
                                        {
                                            echo ' <div class="layout" style= "color:Black">
                                                ...
                                                <span>SB ID: '. $record[0] .'</span><span>; '. $record[12] .'</span>; <span>DOC: '. $record[2] .'</span>
                                                </p>
                                                <p align="center">
