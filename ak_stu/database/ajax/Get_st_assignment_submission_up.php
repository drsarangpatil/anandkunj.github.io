<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT p.full_name, p.at_post FROM ak_p_registration p, ak_st_assignment_submission s WHERE isstudent='Student'");
            try
            {
        ...
        if($process === "getAll_assignment_submission_records")
		{
			$st_course_application_id=filter_input(INPUT_GET, 'st_course_application_id');
            
			//echo "6";
            
            $query = ("SELECT st_assignment_submission_id, d_allo, d_submi, mode_communi, assignment_name, checked_by, st_course_application_id, user_id FROM ak_st_assignment_submission WHERE st_course_application_id  = '" . $st_course_application_id ."'");
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        $row= $row .'<td name=st_assignment_submission_id align="center">' . $record[0] . '</td>';
                        $row= $row .'<td name=d_allo align="center">' . $record[1] . '</td>';
                        $row= $row .'<td name=d_submi align="center">' . $record[2] . '</td>';
                        $row= $row .'<td name=mode_communi align="center">' . $record[3] . '</td>';
                        $row= $row .'<td name=assignment_name align="center">' . $record[4] . '</td>';
                        $row= $row .'<td name=checked_by align="center">' . $record[5] . '</td>';
         ...
         if($process === "getSelected_submission_details")
		{
			$st_assignment_submission_id=filter_input(INPUT_GET, 'st_assignment_submission_id');
            
			//echo "6";
            
			$query = ("SELECT d_allo, d_submi, mode_communi, checked_by, assignment_name, grade_secu, assignment_submitted FROM ak_st_assignment_submission WHERE st_assignment_submission_id = '" . $st_assignment_submission_id . "'");
