<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT p.full_name, p.at_post FROM ak_p_registration p, ak_st_practicals_allocation a WHERE isstudent='Student'");
            try
            {
        ...
        if($process === "getAll_praticals_allocation_records")
		{
			$st_course_application_id=filter_input(INPUT_GET, 'st_course_application_id');
            
			//echo "6";
            
            $query = ("SELECT st_practicals_allocation_id, st_course_application_id, d_allo_pra, mode_communi, practical_name, course_practicals, user_id FROM ak_st_practicals_allocation WHERE st_course_application_id  = '" . $st_course_application_id ."'");
			...
			{ 
				...
				while($record = mysqli_fetch_array($response)) 
				{
                   $row= $row .'<tr class="odd gradeX" >';
                        $row= $row .'<td name=st_practicals_allocation_id align="center">' . $record[0] . '</td>';
                        $row= $row .'<td name=d_allo_pra align="center">' . $record[2] . '</td>';
                        $row= $row .'<td name=mode_communi align="center">' . $record[3] . '</td>';
                        $row= $row .'<td name=practical_name align="center">' . $record[4] . '</td>';
                        $row= $row .'<td name=course_practicals align="center">' . $record[5] . '</td>';
        ...
        if($process === "getSelected_practicals_allocation_data")
		{
			$st_practicals_allocation_id=filter_input(INPUT_GET, 'st_practicals_allocation_id');
            
			//echo "6";
            
			$query = ("SELECT d_allo_pra, mode_communi, practical_name, course_practicals FROM ak_st_practicals_allocation WHERE st_practicals_allocation_id = '" . $st_practicals_allocation_id . "'");
