 $(document).ready
        (
            ...
            {	
                ...
                (
                    {
                        url: 'database/ajax/Get_st_assignment_submission_up.php',	
                        type: 'GET',								
                                            ...
                                            (
                                                {
                                                    url: 'database/ajax/Get_st_assignment_submission_up.php',
                                                    type: 'GET',							
                                                    data: { process: "getAll_assignment_submission_records", st_course_application_id:res_a[0]},

                                                    success: function (data) 
                                                    {
                                                        //alert("Success : " + data);

                                                        $("#course_submission_records").html(data);
...
function myshow()
        {
            ...
            var st_assignment_submission_id = $('#st_assignment_submission_id').val();
            
            //alert(st_assignment_submission_id);
            ...
            (
                {
                    url: 'database/ajax/Get_st_assignment_submission_up.php',
                    type: 'GET',							
                    data: { process: "getSelected_submission_details", st_assignment_submission_id:st_assignment_submission_id},
                    ...
                    success: function (data) 
                    {
                        ...
                        $("#d_allo").val(res_a[0]);
                        $("#d_submi").val(res_a[1]);
                        $("#mode_communi").val(res_a[2]);
                        $("#checked_by").val(res_a[3]);
                        $("#assignment_name").val(res_a[4]);
                        $("#grade_secu").val(res_a[5]);
                        $("#assignment_submitted").val(res_a[6]);
