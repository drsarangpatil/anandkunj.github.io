<?php
	...
	if ($con)
	{
		...
		{
			//echo "4";
			$query = ("SELECT Distinct p.full_name, p.at_post FROM ak_p_registration p, ak_op_treatment_prescription m WHERE p.ispatient='Patient' ");
            try
            {
        ...
        if($process === "getSelected_treatment_prescription_data")
		{
			$op_visit_id=filter_input(INPUT_GET, 'op_visit_id');
			//echo "6";
            
			$query = ("SELECT treatment_name, therapy_dept, treatment_time, therapist_name,treat_instructions FROM ak_op_treatment_prescription WHERE op_visit_id = '" . $op_visit_id. "'");
        ...
        if($process === "getSelected_diet_prescription_data")
		{
			$op_visit_id=filter_input(INPUT_GET, 'op_visit_id');
			//echo "6";
            
			$query = ("SELECT morning_diet, noon_diet, afternoon_diet, evening_diet,instruc FROM ak_op_diet_prescription WHERE op_visit_id = '" . $op_visit_id. "'");
