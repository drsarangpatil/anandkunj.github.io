                            ...
                            <?php 
                                ...
                                try
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        $query="SELECT DISTINCT v.full_name, v.prescription_date, v.age, v.gender, v.final_diagnosis, v.complaints, v.weight, v.bp, v.oe,  m.medicine_names, m.dosage, m.quantity, v.direct_visit_id    FROM  ak_direct_visit v, ak_dir_medicine_prescription m WHERE v.direct_visit_id= m.direct_visit_id";

                                        if( isset($_GET["from_date"]))
                                           if( $_GET["from_date"]!==""  &&  $_GET["to_date"]!=="" )
                                            {
                                                $query = $query .   " and  (v.prescription_date between '".  $_GET["from_date"] . "' and '".  $_GET["to_date"] . "')"; 
                                            }

                                        if( isset($_GET["gender"]))
                                            if( $_GET["gender"]!=="")
                                            {
                                                $query = $query .   " and ( v.gender ='".  $_GET["gender"] . "')  "; 
                                            }
                                        
                                         if( isset($_GET["full_name"]))
                                            if( $_GET["full_name"]!=="")
                                            {
                                                $query = $query .   " and ( v.full_name ='".  $_GET["full_name"] . "')  "; 
                                            }
                                        ...
                                        while($record = mysqli_fetch_array($response))
                                        {
                                             ...
                                             echo '<tr class="odd gradeX" style="font-size:13px" >';
                                                ...
                                                echo '<td align="left"><a href= "print/Dir_medicine_prescription_print.php?vsid=' .$record[12] . '">' . $record[0] . '</a></td>'; 
