<?php
    ...
    for ($i=1; $i<=$records; $i++)
    {
        $index1="medicine_names_id".$i;
        $medicine_names_id[$i]=$_POST[$index1];
        $index2="disease_category".$i;
        $disease_category[$i]=$_POST[$index2];
        $index3="disease_name".$i;
        $disease_name[$i]=$_POST[$index3];
        $index4="medicine_names".$i;
        $medicine_names[$i]=$_POST[$index4];
        $index5="dosage".$i;
        $dosage[$i]=$_POST[$index5];
    }
    ...
    if ($con)
	{
        ...
        for ($i=1; $i<=$records; $i++)
        {
            $query= "UPDATE  ak_medicine_names SET disease_category='$disease_category[$i]', disease_name='$disease_name[$i]', medicine_names='$medicine_names[$i]', dosage='$dosage[$i]' where medicine_names_id=$medicine_names_id[$i]";
