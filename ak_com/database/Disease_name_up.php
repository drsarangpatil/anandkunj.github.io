<?php
    ...
    for ($i=1; $i<=$records; $i++)
    {
        $index1="disease_name_id".$i;
        $disease_name_id[$i]=$_POST[$index1];
    ...
    if ($con)
	{
        ...
        for ($i=1; $i<=$records; $i++)
        {
            $query= "UPDATE ak_disease_name SET disease_category='$disease_category[$i]', disease_name='$disease_name[$i]' where disease_name_id=$disease_name_id[$i]";
	...
	echo "<script> location.replace('../disease_name_up.php'); </script>";
