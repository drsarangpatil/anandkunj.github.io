<?php
	...
	if ($con)
	{
		...
		if($process === "getLanguage")
		{
			...
			//echo "10";
			$query = ("SELECT life_style_instructions_id, language FROM ak_life_style_instructions WHERE disease_category = '" . $disease_category . "' AND disease_name = '" . $disease_name . "'");
			//echo $query;
			$response = mysqli_query($con, $query);
			if(mysqli_num_rows($response)>0)
			{ 
        ...
        if($process === "getLife_style_instructions")
		{
			...
			//echo "12";
			$query = ("SELECT life_style_instructions_id, life_style_instructions FROM ak_life_style_instructions WHERE disease_category = '" . $disease_category . "' AND disease_name = '" . $disease_name . "' AND language = '" . $language . "'");
