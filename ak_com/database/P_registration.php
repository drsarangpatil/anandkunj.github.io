<?php
	...
	if ($con)
	{
		...
		
		{
            ...
            if($_POST["person_id"] !=="")
            {
                $query="UPDATE ak_p_registration SET dor='". $_POST["dor"]."', full_name='". $_POST["full_namea"]."', gender='". $_POST["gender"]."', dob='". $_POST["dob"]."', pob = '". $_POST["pob"]."', marital_status='". $_POST["marital_status"]."', male_child= '". $_POST["male_child"]."', female_child= '". $_POST["female_child"]."', occupation= '". $_POST["occupation"]."', occu_spl_fea='". $_POST["occu_spl_fea"]."', photo= '". $_FILES["photo"]["name"]."', address='". $_POST["address"]."', at_post='". $_POST["at_post"]."', nation_name= '". $_POST["nation_namea"]."', state_name= '". $_POST["state_namea"]."', district_name='". $_POST["district_namea"]."', mobile_no= '". $_POST["mobile_no"]."', emergency_no= '". $_POST["emergency_no"]."', email='". $_POST["email"]."', reference='". $_POST["reference"]."', isparticipant= '". $par."', ispatient= '". $pat."', isstudent= '". $stu."', isemployee= '". $emp."', issubscriber= '". $sub."', ismember= '". $mem."', isdonner= '". $don."', user_id= '". $_POST["user_id"]."' WHERE person_id='" . $_POST["person_id"] ."'";
            }
            else
            {
                //create query
                $query= "INSERT INTO ak_p_registration (dor, full_name,  gender, dob, pob, marital_status, male_child, female_child, occupation, occu_spl_fea, photo, address, at_post, nation_name, state_name, district_name, mobile_no, emergency_no, email, reference, isparticipant, ispatient, isstudent, isemployee, issubscriber, ismember, isdonner, user_id) VALUES ( '". $_POST["dor"]."', '". $_POST["full_namea"]."', '". $_POST["gender"]."', '". $_POST["dob"]."', '". $_POST["pob"]."', '". $_POST["marital_status"]."', '". $_POST["male_child"]."', '". $_POST["female_child"]."', '". $_POST["occupation"]."', '". $_POST["occu_spl_fea"]."', '". $_FILES["photo"]["name"]."', '". $_POST["address"]."', '". $_POST["at_post"]."', '". $_POST["nation_namea"]."', '". $_POST["state_namea"]."', '". $_POST["district_namea"]."', '". $_POST["mobile_no"]."', '". $_POST["emergency_no"]."', '". $_POST["email"]."', '". $_POST["reference"]."',  '". $par."', '". $pat."', '". $stu."', '". $emp."', '". $sub."', '". $mem."', '". $don."',  '". $_POST["user_id"]."')";
            }
